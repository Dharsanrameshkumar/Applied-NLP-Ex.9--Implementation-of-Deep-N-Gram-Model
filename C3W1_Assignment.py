...
def create_batch_dataset(lines, vocab, seq_length=100, batch_size=64):
    ...
    ### START CODE HERE ###
    # Convert lines to tensor of character IDs
    all_ids = line_to_tensor(single_line_data, vocab)
    
    # Create TensorFlow dataset from tensor
    ids_dataset = tf.data.Dataset.from_tensor_slices(all_ids)
    
    # Batch into sequences of length (seq_length + 1)
    data_generator = ids_dataset.batch(seq_length + 1, drop_remainder=True)
    
    # Split each sequence into input and target using the helper
...
class GRULM(tf.keras.Model):
    def __init__(self, vocab_size=256, embedding_dim=256, rnn_units=128):
        ...
        # GRU layer (returns sequences + final state)
        ...
        # Dense output with log softmax activation
...
def log_perplexity(preds, target):
    ...
    Args:
        preds (tf.Tensor): Predictions (log probabilities) of a list of batches of tensors corresponding to lines of text.
    ...
    ### START CODE HERE ###
    # Select the log probabilities of the true target tokens
    log_p = np.sum(tf.one_hot(target, depth=preds.shape[-1]) * preds, axis=-1)
    
    # Mask: 1 for valid tokens, 0 for padding tokens
    non_pad = 1.0 - np.equal(target, PADDING_ID)
    
    # Mask out padding from log probabilities
    log_p = log_p * non_pad
    
    # Compute mean log perplexity
